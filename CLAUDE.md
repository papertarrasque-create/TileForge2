...
Editor (TileGroups, Entities, Map) → F5 → PlayModeController
  ...
  └─ ScreenManager (play-mode-only screen stack)
       ├─ GameplayScreen (movement, collision, hazard, AP combat, entity turn)
...
**Sidebar HUD:** Retro CRPG sidebar (280px, right side) drawn outside game viewport scissor rect. `SidebarHUD` renders player stats (HP/PP bars, ATK/DEF, AP pips, status effects), equipment, inventory, and scrollable message log. `GameLog` (200 max entries) captures all combat, dialogue, and quest events via `LogAndFloat()` pattern in GameplayScreen. Log auto-scrolls (bottom-up viewport fill), supports manual scroll via mouse wheel (3 entries/tick). Word-wrapped text via `TextUtils.WrapText()`. Play mode viewport partitioned: game canvas width = screen - SidebarWidth.
