...
public class ToolPanel : Panel
{
    ...
    public override int PreferredHeight => 3 * (ButtonSize + ButtonPadding) + ButtonPadding;

    private const int ButtonSize = LayoutConstants.ToolButtonSize;
    private const int ButtonPadding = LayoutConstants.ToolButtonPadding;

    private static readonly Color ButtonColor = LayoutConstants.ToolButtonColor;
    private static readonly Color ButtonActiveColor = LayoutConstants.ToolButtonActiveColor;
    private static readonly Color ButtonHoverColor = LayoutConstants.ToolButtonHoverColor;
    private static readonly Color IconColor = LayoutConstants.ToolIconColor;
    private static readonly Color IconDimColor = LayoutConstants.ToolIconDimColor;
                                        ...
                                        SpriteFont font, GameTime gameTime, int screenW, int screenH)
    {
        ...
        if (mouse.LeftButton == ButtonState.Pressed && prevMouse.LeftButton == ButtonState.Released)
        {
            switch (hitIndex)
            {
                ...
                case 4: state.ActiveTool = new PickerTool(); break;
                case 5: state.ActiveTool = new SelectionTool(); break;
            }
        }
    }
                                      ...
                                      int toolIndex, Rectangle btnRect, Color color)
    {
        ...
        switch (toolIndex)
        {
            ...
            case 4: DrawPickerIcon(spriteBatch, renderer, cx, cy, color); break;
            case 5: DrawSelectionIcon(spriteBatch, renderer, cx, cy, color); break;
        }
    }
    ...
    {
        // Eyedropper shape: tip at bottom, body angled up
        r.DrawRect(sb, new Rectangle(cx - 1, cy + 5, 2, 4), c);    // Dropper tip
        r.DrawRect(sb, new Rectangle(cx - 2, cy + 1, 4, 4), c);    // Dropper lower body
        r.DrawRect(sb, new Rectangle(cx - 3, cy - 3, 6, 4), c);    // Dropper upper body
        r.DrawRect(sb, new Rectangle(cx - 2, cy - 7, 4, 4), c);    // Bulb
        r.DrawRect(sb, new Rectangle(cx - 1, cy - 9, 2, 2), c);    // Bulb top
    }

    private static void DrawSelectionIcon(SpriteBatch sb, Renderer r, int cx, int cy, Color c)
    {
        // Dashed rectangle selection shape
        r.DrawRectOutline(sb, new Rectangle(cx - 7, cy - 7, 14, 14), c, 1);
        // Dashed effect: small corner marks
        r.DrawRect(sb, new Rectangle(cx - 7, cy - 7, 4, 1), c);    // Top-left H
        r.DrawRect(sb, new Rectangle(cx - 7, cy - 7, 1, 4), c);    // Top-left V
        r.DrawRect(sb, new Rectangle(cx + 3, cy - 7, 4, 1), c);    // Top-right H
        r.DrawRect(sb, new Rectangle(cx + 6, cy - 7, 1, 4), c);    // Top-right V
        r.DrawRect(sb, new Rectangle(cx - 7, cy + 3, 1, 4), c);    // Bottom-left V
        r.DrawRect(sb, new Rectangle(cx - 7, cy + 6, 4, 1), c);    // Bottom-left H
        r.DrawRect(sb, new Rectangle(cx + 6, cy + 3, 1, 4), c);    // Bottom-right V
        r.DrawRect(sb, new Rectangle(cx + 3, cy + 6, 4, 1), c);    // Bottom-right H
    }
}
